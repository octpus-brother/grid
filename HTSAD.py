...
class HTSAD(nn.Module):
    def __init__(self, Dim, Embedding_sz, Output_sz, Eventfilter_sz, Event_num, Categorical_num, Numerical_num, Device):
        ...
        self.Eventfilter_sz = Eventfilter_sz
        self.Device = Device
    ...
    def forward(self, event, time, vc, vn, init_states=None):
        ...
        if init_states is None:
            h_t, c_t = (torch.zeros(bs, self.Output_sz).to(self.Device), torch.zeros(bs, self.Output_sz).to(self.Device))
        else:
            h_t, c_t = init_states
        HS = self.Output_sz
        for t in range(seq_sz):
            ...
            es = self.eventfilter2(self.eventfilter1(s))
            es = self.eventfilter4(self.eventfilter3(es))
            j_t = es
...
def train(args, device, trainlogs):
    ...
    for epoch in range(args["Epoches"]):
        ...
        for s, t, vc, vn in train_loader:
            ...
            if count % 100 == 1:
                ...
                print("        loss = {:.4f}\t{:.2%}".format(loss.data, progress))
            # else:
    ...
    if not os.path.exists('./models/HTSAD/epoch{}/{}/'.format(modelnum, severname)):
        os.makedirs('./models/HTSAD/epoch{}/{}/'.format(modelnum, severname))
    torch.save(layer.state_dict(), './models/HTSAD/epoch{}/{}/HTSADmodel.pth'.format(modelnum, severname))
    torch.save(optimizer.state_dict(), './models/HTSAD/epoch{}/{}/HTSADoptim.pth'.format(modelnum, severname))
    f = open('./models/HTSAD/epoch{}/{}/HTSADcenter&args.pkl'.format(modelnum, severname), 'wb')
    pickle.dump(center, f)
    pickle.dump(args, f)
    f.close()
    f = open("./models/HTSAD/epoch{}/{}/HTSADtrain.csv".format(modelnum, severname), "w")
    writer = csv.writer(f)
    for i in args:
...
def test(test_args, device, testlogs):
    ...
    f = open('./models/HTSAD/epoch{}/{}/HTSADcenter&args.pkl'.format(modelnum, severname), 'rb')
    ...
    layer.load_state_dict(torch.load('./models/HTSAD/epoch{}/{}/HTSADmodel.pth'.format(modelnum, severname)))
    layer.to(device)
    ...
    f = open("./return/epoch{}/{}/HTSADtest.csv".format(modelnum, severname), "w")
    writer = csv.writer(f)
    for i in loss_seq:
        writer.writerow((str(i),))
    f.close()
